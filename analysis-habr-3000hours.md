...
Статья описывает **естественную эволюцию**: от «построить всё самому» к «интегрировать лучшее из экосистемы». Но это НЕ «выбросить и заменить» — это **гибридный подход**:

**Взять из статьи:**
- Beads для трекинга задач (главный missing piece)
- Отказ от over-engineering изоляции (1M контекст)
- Упрощение пайплайна (методология вместо жёстких хуков)

**Сохранить своё:**
- Language hooks, doc-drafts, observer, config paths, deploy.sh, knowledge-harvest, find-skills

**Результат:** Lightweight Orchestration Kit = language hooks + doc-drafts + observer + config + deploy, работающий ПОВЕРХ Superpowers + Beads + LightRAG.
