...
- **«Набор агентов И моделей»** = папка `~/.codex/agents/*.toml`, где в каждом worker-файле зашиты `model`, `model_reasoning_effort`, `sandbox_mode`, `developer_instructions`. Текущий чат остаётся координатором; отдельный `orchestrator.toml` не ставится.
