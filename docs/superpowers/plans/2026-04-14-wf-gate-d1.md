...
**New files:**
- `templates/commands/wf-gate.md` — slash-command entry point

**Modified files:**
- `templates/skills/wf-gate/SKILL.md` — shrink Phases 1 and 5; add 4th point to Phase 4 close reason; rewrite preamble
...
## Task 1: Create `/wf-gate` slash command

**Files:**
- Create: `templates/commands/wf-gate.md`

- [ ] **Step 1: Verify file does not exist (baseline)**

Run: `ls /root/projects/orchestration-kit/templates/commands/wf-gate.md 2>&1`
...
Path: `/root/projects/orchestration-kit/templates/commands/wf-gate.md`
...
1. **Beads create** — use 6-point description (see wf-gate skill § Phase 2):
   what, where in code, how to reproduce, what's found, context, resources.

2. **Beads close** — use 4-point reason (see wf-gate skill § Phase 4):
...
Run: `head -5 /root/projects/orchestration-kit/templates/commands/wf-gate.md && echo '---' && grep -c 'unified-workflow\|4-point\|6-point\|$ARGUMENTS' /root/projects/orchestration-kit/templates/commands/wf-gate.md`
...
git add templates/commands/wf-gate.md
git commit -m "feat: add /wf-gate slash command (delegates to template-bridge:unified-workflow)"
```

---

## Task 2: Rewrite `wf-gate` skill preamble and description

**Files:**
- Modify: `templates/skills/wf-gate/SKILL.md` (lines 1-16)

- [ ] **Step 1: Read current preamble (baseline)**

Run: `sed -n '1,16p' /root/projects/orchestration-kit/templates/skills/wf-gate/SKILL.md`
...
description: >
  ...
  Dev-loop orchestration lives in slash-command /wf-gate, which delegates
...
Run: `sed -n '1,20p' /root/projects/orchestration-kit/templates/skills/wf-gate/SKILL.md && echo --- && grep -c 'workflow-active\|unlock' /root/projects/orchestration-kit/templates/skills/wf-gate/SKILL.md`
...
git commit -m "refactor(wf-gate): rewrite preamble — skill becomes Beads-discipline reference"
```
...
**Files:**
- Modify: `templates/skills/wf-gate/SKILL.md` — remove current Phase 1 block
...
Run: `grep -n '^## Phase\|^### [0-9]\.' /root/projects/orchestration-kit/templates/skills/wf-gate/SKILL.md`
...
Run: `grep -n 'workflow-active\|Phase 1\|Activate gate\|Launch unified workflow' /root/projects/orchestration-kit/templates/skills/wf-gate/SKILL.md`
...
cd /root/projects/orchestration-kit
git add templates/skills/wf-gate/SKILL.md
git commit -m "refactor(wf-gate): remove Phase 1 (marker + unified-workflow delegation)"
```
...
**Files:**
- Modify: `templates/skills/wf-gate/SKILL.md`, Phase 5 "Land the Plane" section

- [ ] **Step 1: Locate marker-removal step in Phase 5**

Run: `grep -n 'Маркер\|workflow-active\|rm -f' /root/projects/orchestration-kit/templates/skills/wf-gate/SKILL.md`
...
Run: `grep -c 'workflow-active' /root/projects/orchestration-kit/templates/skills/wf-gate/SKILL.md`
Expected: `0`
...
cd /root/projects/orchestration-kit
git add templates/skills/wf-gate/SKILL.md
git commit -m "refactor(wf-gate): remove marker cleanup from Session End"
```
...
**Files:**
- Modify: `templates/skills/wf-gate/SKILL.md`, top of Phase 3 "During Work"
...
Run: `grep -n '^## Phase 3' /root/projects/orchestration-kit/templates/skills/wf-gate/SKILL.md`
...
Run: `sed -n '/## Phase 3/,/## Phase 4/p' /root/projects/orchestration-kit/templates/skills/wf-gate/SKILL.md | head -30`
...
cd /root/projects/orchestration-kit
git add templates/skills/wf-gate/SKILL.md
git commit -m "refactor(wf-gate): move bd prime/recall/ready into Phase 3 as 3.0"
```
...
**Files:**
- Modify: `templates/skills/wf-gate/SKILL.md`, Phase 4 "Closing — Quality Standard"

- [ ] **Step 1: Locate current 3-point structure**

Run: `sed -n '/## Phase 4/,/^## Phase 5/p' /root/projects/orchestration-kit/templates/skills/wf-gate/SKILL.md | head -50`
...
Run: `grep -c '4 обязательных\|Verification\|verification-before-completion' /root/projects/orchestration-kit/templates/skills/wf-gate/SKILL.md`
Expected: `>= 3` (heading + point 4 name + skill reference).

Also: `grep -n '3 обязательных' /root/projects/orchestration-kit/templates/skills/wf-gate/SKILL.md`
Expected: no matches.
...
cd /root/projects/orchestration-kit
git add templates/skills/wf-gate/SKILL.md
git commit -m "feat(wf-gate): expand close-reason to 4-point with mandatory Verification evidence"
```
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          ...
          {
            "type": "command",
            "command": "echo 'Workflow: /wf-gate <task> (delegates to template-bridge:unified-workflow). Beads discipline: wf-gate skill. Verification: superpowers:verification-before-completion (Iron Law).'"
          }
        ]
      }
    ],
...
- If the surrounding flow describes orchestration, replace with reference to slash-command `/wf-gate` → unified-workflow.

Specifically:
- Any block like `touch .workflow-active` → delete
- Any diagram arrow like `├─ touch .workflow-active (разблокирует Edit/Write)` → delete
- Any sentence like "Edit/Write заблокированы пока не вызван..." → replace with: "`/wf-gate <task>` — slash-команда, делегирует `template-bridge:unified-workflow` и добавляет наши Beads-стандарты (6-point description, 4-point close reason с verification evidence)."
- Any flow diagram with `unified-workflow (Template Bridge)` as a sub-step of marker creation → replace with `/wf-gate` as the entry point.
...
```bash
# AC1: /wf-gate exists as command
test -f templates/commands/wf-gate.md && echo 'AC1 ✓'
...
# AC4: SessionStart echo has /wf-gate
grep -q '/wf-gate' templates/settings-hooks.json && echo 'AC4 ✓'

# AC5: skill is Beads-centric
grep -q 'Beads-discipline reference' templates/skills/wf-gate/SKILL.md && echo 'AC5 ✓'

# AC6: 4-point close reason
grep -q '4 обязательных' templates/skills/wf-gate/SKILL.md && echo 'AC6 ✓'
